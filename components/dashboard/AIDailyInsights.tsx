...
export const AIDailyInsights: React.FC = () => {
   ...
   const generateInsights = async () => {
      ...
      `;
         ...
         const data = await aiService.generateJson(prompt, "System: Business Intelligence Node Analysis");
         if (data) {
            setInsightData(data);
         }
      } catch (error) {
                              ...
                              {insightData.kpis?.map((kpi: any, i: number) => (
                                 <div key={i} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col gap-2">
                                    <div className="flex items-center justify-between">
                                       ...
                                       <div className={`w-2 h-2 rounded-full ${kpi.status === 'positive' ? 'bg-emerald-500' :
