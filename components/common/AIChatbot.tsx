...
export const AIChatbot: React.FC = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: aiText || "I'm having trouble processing that right now. How else can I help your shop today?", timestamp: new Date() }]);
    } catch (error) {
            ...
            {messages.map((m, idx) => (
              <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                <div className={`flex gap-3 max-w-[85%] ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`p-4 rounded-2xl text-xs font-medium leading-relaxed shadow-sm ${m.role === 'user'
