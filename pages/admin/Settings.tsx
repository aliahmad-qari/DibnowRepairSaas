...
export const AdminSettings: React.FC = () => {
  ...
  const invokeAiAudit = async () => {
    ...
    try {
      ...
      const data = await aiService.generateJson(prompt, "System: Senior SaaS Infrastructure AI Auditor Snapshot Analysis");
      if (data) {
        setAiReport(data);
      }
    } catch (error) {
                      ...
                      {aiReport.insights?.map((insight: any, i: number) => (
                        <div key={i} className="bg-slate-50 border-2 border-slate-100 p-8 rounded-[2.5rem] flex flex-col md:flex-row items-start gap-8 group hover:border-indigo-200 transition-all">
                          <div className="flex flex-col items-center gap-3 shrink-0">
                            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg ${insight.risk_level === 'High' ? 'bg-rose-600 text-white shadow-rose-100' :
                                insight.risk_level === 'Medium' ? 'bg-amber-500 text-white shadow-amber-100' :
                                  'bg-indigo-600 text-white shadow-indigo-100'
                              }`}>
                              <AlertTriangle size={32} />
                            </div>
                            <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${insight.risk_level === 'High' ? 'text-rose-600 bg-rose-50' :
