...
export const PlanRequests: React.FC = () => {
  ...
  useEffect(() => {
    const loadRequests = async () => {
      try {
        const data = await callBackendAPI('/api/plan-requests', null, 'GET');
        setRequests(data);
      } catch (error) {
  ...
  const handleDeny = async (id: string) => {
    if (window.confirm("Are you sure you want to deny this request?")) {
      try {
        await callBackendAPI(`/api/plan-requests/${id}/status`, { status: 'denied' }, 'PUT');
        // Refresh list
        const data = await callBackendAPI('/api/plan-requests', null, 'GET');
        setRequests(data);
      } catch (error) {
  ...
  const handleFinalApproval = async (e: React.FormEvent) => {
    ...
    try {
      await callBackendAPI(`/api/plan-requests/${approvalRequest._id}/status`, {
        status: 'approved',
        invoiceStatus: approvalMeta.invoiceStatus,
        planStatus: approvalMeta.planStatus
      }, 'PUT');
  ...
  const filteredRequests = requests.filter(req =>
    ...
    (req._id || req.id || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredRequests.map(req => (
                <tr key={req._id || req.id} className="hover:bg-indigo-50/30 transition-all group">
                  ...
                  <td className="px-10 py-7 text-center">
                    <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase inline-flex items-center gap-2 border ${req.status === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-100' :
                      req.status === 'approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-rose-50 text-rose-700 border-rose-100'
                      }`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${req.status === 'pending' ? 'bg-amber-500' : req.status === 'approved' ? 'bg-emerald-500' : 'bg-rose-500'
                        }`} />
                      {req.status}
                    </span>
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-3">
                      {req.status === 'pending' ? (
                        <>
                          ...
                          <button
                            onClick={() => handleDeny(req._id || req.id)}
