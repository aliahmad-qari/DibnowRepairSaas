...
export const SecurityIntelligence: React.FC = () => {
   ...
   const invokeAiAudit = async () => {
      ...
      `;
         ...
         const data = await aiService.generateJson(prompt, "System: Senior SaaS Security Auditor Advisory");
         if (data) {
                        ...
                        filteredFlags.map((flag) => (
                           <tr key={flag.id} className="hover:bg-rose-50/30 transition-all group cursor-default h-24">
                              ...
                              <td className="px-6 text-center">
                                 <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border inline-flex items-center gap-2 ${flag.risk === 'Critical' ? 'bg-rose-50 text-rose-700 border-rose-100' :
                                       flag.risk === 'Warning' ? 'bg-amber-50 text-amber-700 border-amber-100' :
                                          'bg-blue-50 text-blue-700 border-blue-100'
                                    }`}>
                                    <div className={`w-1.5 h-1.5 rounded-full ${flag.risk === 'Critical' ? 'bg-rose-500 animate-pulse' :
                     ...
                     {aiInsights.map((insight, idx) => (
                        <div key={idx} className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] flex flex-col gap-6 group hover:border-indigo-500/50 transition-all">
                           <div className="flex items-center justify-between">
                              ...
                              <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded border ${insight.severity === 'High' ? 'bg-rose-50/10 text-rose-400 border-rose-500/20' :
