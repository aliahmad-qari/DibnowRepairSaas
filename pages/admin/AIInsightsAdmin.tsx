...
export const AdminAIInsights: React.FC = () => {
   ...
   const invokeAIAudit = async () => {
      ...
      `;
         ...
         const data = await aiService.generateJson(prompt, "System: Senior SaaS Platform Architect Global Audit");
         if (data) {
            setReport(data);
         }
      } catch (e) {
                        ...
                        {report.risk_nodes?.map((risk: any, i: number) => (
                           <div key={i} className="p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm space-y-4 hover:border-indigo-300 transition-all group">
                              <div className="flex items-center justify-between">
                                 ...
                                 <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border ${risk.level === 'High' ? 'bg-rose-50 text-rose-700 border-rose-100 animate-pulse' : 'bg-slate-50 text-slate-500 border-slate-100'
