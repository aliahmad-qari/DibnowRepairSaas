    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 relative px-4 sm:px-0 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <BackButton />
          <div>
            <h2 className="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight uppercase">Brand intelligence</h2>
            <div className="flex flex-wrap items-center gap-3 mt-1">
              <p className="text-slate-500 font-bold text-[9px] sm:text-[10px] uppercase tracking-widest">Manufacturer Ecosystem</p>
        ...
        <button
          ...
          className={`${isAtLimit ? 'bg-slate-800' : 'bg-indigo-600'} text-white px-8 py-4 rounded-2xl font-black flex items-center justify-center gap-2 shadow-xl hover:scale-105 active:scale-95 transition-all text-[10px] uppercase tracking-widest w-full lg:w-auto`}
        >
      ...
      {intel.deadBrands.length > 0 && brands.length > 0 && (
        <div className="bg-rose-50 border-2 border-rose-100 p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2.5rem] flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in slide-in-from-top-2">
          <div className="flex items-center gap-5">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-rose-600 text-white rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shrink-0">
              <ShieldAlert size={20} sm:size={24} />
            </div>
            <div>
              <h4 className="text-xs sm:text-sm font-black text-rose-900 uppercase">Underperforming brands</h4>
              <p className="text-[9px] sm:text-[10px] font-bold text-rose-600 uppercase tracking-widest mt-1">⚠ {intel.deadBrands.length} brands have no activity detected.</p>
            </div>
          </div>
          <button onClick={() => setActiveFilter('inactive')} className="w-full sm:w-auto px-6 py-3 bg-white border border-rose-200 text-rose-600 rounded-xl text-[10px] font-black uppercase hover:bg-rose-100 transition-all">Review</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-8 space-y-8">
          {/* Add Form Section */}
          {showForm && !isAtLimit && (
            <div className="bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-indigo-100 shadow-2xl animate-in zoom-in-95 duration-200 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none rotate-12">
                <Tag size={150} />
              </div>
              <form onSubmit={handleSubmit} className="relative z-10 space-y-6">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-lg sm:text-xl font-black text-slate-800 uppercase tracking-tight">Brand architect</h3>
                ...
                <div className="pt-4 flex flex-col sm:flex-row gap-3">
                  <button type="submit" disabled={isSubmitting} className="flex-[2] bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-indigo-700 transition-all uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 disabled:opacity-50">
                ...
                <div className="overflow-x-auto custom-scrollbar">
                  <table className="w-full text-left">
                    <thead className="bg-slate-50/50 text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] border-b border-slate-100 whitespace-nowrap">
                      <tr>
                        <th className="px-6 sm:px-10 py-6">Manufacturer Node</th>
                        ...
                        <th className="px-6 sm:px-10 py-6 text-right">Heat Index</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {intel.filtered.length === 0 ? (
                        ...
                        intel.filtered.map((brand) => (
                          <tr key={brand._id} className="hover:bg-indigo-50/30 transition-all group whitespace-nowrap">
                            <td className="px-6 sm:px-10 py-7">
                              <div className="flex items-center gap-4">
                                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-indigo-50 text-indigo-600 rounded-xl sm:rounded-2xl flex items-center justify-center font-black text-lg sm:text-xl shadow-sm border border-indigo-100 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                                  {brand.name.charAt(0)}
                                </div>
