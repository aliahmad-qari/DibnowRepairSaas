...
export const Brands: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = () => {
      const allBrands = db.brands.getAll();
      ...
      if (user) {
        ...
        setActivePlan(plan);
        setIsAtLimit(allBrands.length >= plan.limits.brands);
      }
    };
    loadData();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newBrand.name || isAtLimit) return;
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-indigo-100 shadow-2xl animate-in zoom-in-95 duration-200 relative overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="relative z-10 space-y-6">
                ...
                <div className="pt-4 flex gap-3">
                  <button type="submit" className="flex-1 bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-indigo-700 transition-all uppercase tracking-widest text-[10px] flex items-center justify-center gap-2">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {intel.filtered.length === 0 ? (
                        ...
                        intel.filtered.map((brand) => (
                          <tr key={brand.id} className="hover:bg-indigo-50/30 transition-all group">
                            ...
                            <td className="px-6 py-7 text-center">
                              <span className={`px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${
                                brand.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 
