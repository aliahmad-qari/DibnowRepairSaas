...
export const Inventory: React.FC = () => {
  ...
  const comparisonData = useMemo(() => {
    ...
    sales.forEach(s => {
      const item = items.find(i => i.id === s.productId);
  ...
  const intel = useMemo(() => {
    ...
    const fastMoving = [...items]
      .map(i => ({ ...i, salesCount: salesMap[i.id] || 0 }))
    ...
    const profitability = items.map(i => {
      ...
      const totalSold = salesMap[i.id] || 0;
      return {
        ...i,
        cost,
        margin,
  ...
  const handleSellSubmit = (e: React.FormEvent) => {
    ...
    setIsSelling(true);
    setTimeout(() => {
      const success = db.inventory.sell(productToSell.id, sellForm.qty, sellForm.price, sellForm.customer);
      if (success) {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    db.inventory.update(itemToEdit.id, {
      ...editForm,
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
           ...
           {viewMode === 'table' ? (
                           ...
                           <tr key={item.id} className="hover:bg-indigo-50/30 transition-all group">
                              ...
                              <td className="px-10 py-7 text-right">
                                 <div className="flex items-center justify-end gap-2">
                                    ...
                                    <button onClick={() => handleDeleteItem(item.id)} className="p-2.5 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm border border-rose-100"><Trash2 size={16}/></button>
                                 </div>
                              </td>
                           </tr>
                         ))}
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {filteredItems.map(item => (
                  <div key={item.id} className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm hover:shadow-2xl transition-all group flex flex-col h-full border-b-4 border-b-indigo-500">
                    <div className="relative aspect-square bg-slate-50 overflow-hidden">
                       ...
                       <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                          ...
                          <button onClick={() => handleDeleteItem(item.id)} className="p-3 bg-white/90 backdrop-blur text-rose-600 rounded-2xl shadow-xl hover:bg-rose-600 hover:text-white transition-all"><Trash2 size={16}/></button>
                       </div>
                    </div>
                    <div className="p-6">
