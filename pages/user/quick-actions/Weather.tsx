  ...
  const fetchWeatherData = async (lat?: number, lon?: number) => {
    try {
      ...
      if (data && data.current) {
        ...
        // --- ADDITIVE: CACHING ---
        localStorage.setItem('dibnow_weather_cache', JSON.stringify({
        ...
        setIsOffline(false);
        db.activity.log({ actionType: 'Atmospheric Sync', moduleName: 'Meteorological', refId: 'WTHR_NODE', status: 'Success' });
      }
    } catch (err) {
      console.error("Satellite Handshake Failure:", err);
      // --- ADDITIVE: OFFLINE CACHE RESTORATION ---
      const cached = localStorage.getItem('dibnow_weather_cache');
      if (cached) {
