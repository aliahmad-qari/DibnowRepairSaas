...
export const HistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    const repairs = db.repairs.getAll().map(r => ({ 
      ...r, 
    ...
    const walletTx = db.wallet.getTransactions().map(t => ({
      ...t,
      protocolType: 'PAYMENT_EVENT',
      ...
      ipAddress: '127.0.0.1',
      financialValue: t.amount,
      financialType: t.type,
      gateway: t.description.includes(':') ? t.description.split(':')[1].trim() : 'System',
      drillDown: {
        description: `Treasury flux authorized. Type: ${t.type.toUpperCase()}. Protocol: ${t.description}.`,
        before: `${currency.symbol}${(db.user.get()?.walletBalance || 0) - (t.type === 'credit' ? t.amount : -t.amount)}`,
        after: `${currency.symbol}${db.user.get()?.walletBalance || 0}`,
        refId: t.id,
    ...
    const activity = db.activity.getAll().map(a => {
    ...
    const unified = [...repairs, ...sales, ...walletTx, ...activity].sort((a, b) => 
                      ...
                      >
                        ...
                        <td className="px-10 py-7">
                          <div className="flex flex-col gap-1">
                             ...
                             <span className={`text-[8px] font-black uppercase tracking-widest ml-8 ${
                               log.actorType === 'AI (Insight)' ? 'text-violet-500' : 
