...
export const ProfilePage: React.FC = () => {
  ...
  const loginHistory = useMemo(() => {
    ...
    return activity
      .filter(a => a.userId === user?.id && a.actionType.includes('Login'))
      .slice(0, 5)
      .map(log => ({
        id: log.id,
  ...
  const usage = useMemo(() => {
    ...
    const stock = db.inventory.getAll().length;
    const team = db.userTeamV2.getByOwner(user.id).length;

    return {
      planName: plan.name,
      metrics: [
        { label: 'Repairs Used', used: repairs, limit: plan.limits.repairsPerMonth },
        { label: 'Stock Used', used: stock, limit: plan.limits.inventoryItems },
        { label: 'Team Members', used: team, limit: plan.limits.teamMembers },
  ...
  const billingAlerts = useMemo(() => {
    ...
    alerts.push({ id: 'card_expiry', label: 'Card expiring in 14 days', icon: CreditCard, color: 'amber' });
  ...
  const securityStats = useMemo(() => {
    const activity = db.activity.getAll();
    const userLogins = activity.filter(a => a.userId === user?.id && a.actionType.includes('Login'));
    const lastLogin = userLogins[0]?.timestamp || new Date().toISOString();
    
    return {
      lastLogin,
      activeSessions: 1, 
      ipSource: '192.168.1.' + Math.floor(Math.random() * 255)
    };
  }, [user]);
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      ...
      // Update Database
      db.users.update(user.id, { avatar: base64Image });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in duration-500 pb-20 px-4 md:px-0">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Registry Identity</h2>
