...
export const AllStock: React.FC = () => {
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    db.inventory.update(editingItem.id, editingItem);
    setEditingItem(null);
  };
  ...
  const handleSellSubmit = async (e: React.FormEvent) => {
    ...
    const success = db.inventory.sell(isSelling.id, sellForm.qty, sellForm.price, sellForm.customer);
    if (success) {
      setIsSelling(null);
  ...
  const detailAnalysis = useMemo(() => {
    ...
    const itemSales = allSales.filter(s => s.productId === selectedStockDetail.id);
    const itemLogs = allActivity.filter(l => l.refId === selectedStockDetail.id);
    ...
    <div className="space-y-8 pb-32 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
        {filteredItems.map(item => (
          <div key={item.id} className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 group flex flex-col h-full">
            <div className="relative aspect-square bg-slate-50 overflow-hidden">
               ...
               <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0 duration-300">
                  ...
                  <button onClick={() => handleDelete(item.id)} className="p-3 bg-white/90 backdrop-blur text-rose-600 rounded-2xl shadow-xl hover:bg-rose-600 hover:text-white transition-all"><Trash2 size={16}/></button>
               </div>
