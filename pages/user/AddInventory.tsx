...
import { 
  ...
  Info, ShoppingCart, Box, ClipboardList
} from 'lucide-react';
...
export const AddInventory: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setActivePlan(plan);
      const currentStockCount = db.inventory.getAll().length;
      if (currentStockCount >= plan.limits.inventoryItems) {
        setIsLimitReached(true);
      }
      
      // Load Dynamic Options
      setAvailableBrands(db.brands.getAll());
      setAvailableCategories(db.categories.getAll());
    }
  }, [user]);
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 md:p-12 rounded-[3rem] border border-slate-100 shadow-2xl space-y-12">
        ...
        <div className="pt-12 border-t border-slate-50 flex flex-col sm:flex-row gap-4">
          <button type="submit" className="flex-1 bg-[#0052FF] text-white font-black py-6 rounded-3xl shadow-2xl shadow-blue-200 hover:bg-blue-600 hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-[0.3em] text-[11px] flex items-center justify-center gap-3">
            <Save size={20} /> Authorize Stock Enrollment
          </button>
