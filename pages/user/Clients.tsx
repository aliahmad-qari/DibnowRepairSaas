...
export const Clients: React.FC = () => {
  ...
  const intel = useMemo(() => {
    ...
    let stats = clients.map(client => {
      ...
      const clientComplaints = complaints.filter(c => c.user === client.name || c.userId === client.id);
    ...
    const topSpenders = [...stats].sort((a, b) => b.totalSpend - a.totalSpend).slice(0, 5).map(c => c.id);
    const topRepairClients = [...stats].sort((a, b) => b.repairCount - a.repairCount).slice(0, 5).map(c => c.id);

    stats = stats.map(s => ({
      ...s,
      isHighValue: topSpenders.includes(s.id) || topRepairClients.includes(s.id)
    }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    db.clients.add({ ...newClient, createdAt: new Date().toISOString() });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-[1600px] mx-auto">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-indigo-100 shadow-2xl animate-in zoom-in-95 duration-200">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-4 flex gap-3">
              <button type="submit" className="flex-1 bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-indigo-700 transition-all uppercase tracking-widest text-[11px]">Authorize Enrollment</button>
              <button type="button" onClick={() => setShowForm(false)} className="px-8 py-4 bg-slate-100 text-slate-500 font-black rounded-2xl hover:bg-slate-200 transition-all uppercase tracking-widest text-[11px]">Discard</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden border-b-8 border-b-indigo-600 min-h-[400px] flex flex-col">
        {clients.length === 0 ? (
                      ...
                      >
                        ...
                        <td className="px-6 py-7 text-center">
                           <div className="flex flex-col items-center">
                              ...
                              <span className={`text-[8px] font-bold uppercase mt-1 px-2 py-0.5 rounded ${
                                client.daysSinceLastVisit > 90 ? 'bg-rose-50 text-rose-600' :
                        ...
                        <td className="px-6 py-7 text-center">
                          <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border flex items-center justify-center gap-2 mx-auto w-fit ${
                            client.loyalty === 'Loyal' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 
           ...
           <div className="bg-white w-full max-w-2xl h-full shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col relative z-10 overflow-hidden rounded-l-[2rem]">
              <div className="p-10 bg-slate-900 text-white flex flex-col gap-6 shrink-0 relative overflow-hidden">
                 ...
                 <div className="relative z-10">
                    ...
                    <p className="text-[10px] font-bold text-indigo-300 uppercase tracking-[0.3em] mt-3">Entity Audit Reference: ID-{selectedClientForTimeline.id.slice(-6)}</p>
                 </div>
