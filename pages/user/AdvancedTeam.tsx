...
export const AdvancedTeam: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      setMembers(db.userTeamV2.getByOwner(user.id));
    }
  }, [user]);
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;
    db.userTeamV2.add({ ...formData, ownerId: user.id });
    setMembers(db.userTeamV2.getByOwner(user.id));
  ...
  const toggleStatus = (id: string, current: string) => {
    ...
    if (user) setMembers(db.userTeamV2.getByOwner(user.id));
  };
  ...
  return (
    <div className="space-y-8 pb-32 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {members.map(member => (
          <div key={member.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
            ...
            <div className="flex items-start justify-between relative z-10 mb-8">
              ...
              <div className="flex gap-2">
                 <button onClick={() => toggleStatus(member.id, member.status)} className={`p-3 rounded-xl transition-all ${member.status === 'enabled' ? 'bg-rose-50 text-rose-600 hover:bg-rose-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}>
                    <Lock size={18} />
                 </button>
            ...
            <div className="mt-8 pt-8 border-t border-slate-50 flex items-center justify-between relative z-10">
               ...
               <span className="text-[9px] font-black text-slate-300 uppercase italic">ID: {member.id}</span>
            </div>
          </div>
        ))}
      </div>
           ...
           <div className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
              ...
              <div className="overflow-y-auto p-8 md:p-12 custom-scrollbar">
                 <form onSubmit={handleAddSubmit} className="space-y-10">
                    ...
                    <button type="submit" className="w-full bg-blue-600 text-white font-black py-6 rounded-3xl shadow-2xl hover:bg-blue-700 transition-all uppercase tracking-[0.3em] text-[11px] flex items-center justify-center gap-3 active:scale-95">
                       <ShieldCheck size={20} /> Authorize Associate Node
                    </button>
                 </form>
              </div>
           </div>
