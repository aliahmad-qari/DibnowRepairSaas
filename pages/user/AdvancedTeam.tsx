        ...
        <BackButton />
         ...
         <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="text-center lg:text-left">
               <h1 className="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight leading-none uppercase">Advanced Team Node</h1>
               <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-2 flex items-center justify-center lg:justify-start gap-2">
                  <ShieldCheck size={12} className="text-blue-500 shrink-0" /> Granular Access Control System (RBAC)
               </p>
            </div>
            <button
               onClick={() => setShowAddModal(true)}
               className="w-full lg:w-auto bg-blue-600 text-white px-8 py-4 rounded-xl sm:rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl hover:scale-105 transition-all text-[10px] sm:text-[11px] uppercase tracking-widest"
            >
               <UserPlus size={18} /> Provision V2 Associate
            </button>
         </div>

         <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 relative min-h-[400px]">
            {isLoading && (
            ...
            {members.map(member => (
               <div key={member._id} className="bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
                  <div className={`absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 rounded-full -mr-12 sm:-mr-16 -mt-12 sm:-mt-16 transition-transform duration-700 group-hover:scale-150 ${member.status === 'enabled' ? 'bg-blue-50' : 'bg-rose-50'}`} />

                  <div className="flex items-start justify-between relative z-10 mb-6 sm:mb-8">
                     <div className="flex items-center gap-4 sm:gap-5">
                        <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-[1.5rem] flex items-center justify-center text-lg sm:text-xl font-black shadow-lg shrink-0 ${member.status === 'enabled' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-400'}`}>
                           {member.name.charAt(0)}
                        </div>
                        <div>
                           <h3 className="text-lg sm:text-xl font-black text-slate-800 tracking-tight uppercase leading-tight">{member.name}</h3>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                     {Object.entries(member.permissions || {}).map(([module, perms]: [string, any]) => (
                        <div key={module} className="bg-slate-50 p-4 rounded-xl sm:rounded-2xl border border-slate-100">
                           <p className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 border-b border-slate-200 pb-1">{module}</p>
                  ...
                  <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10">
                     ...
                     <span className="text-[8px] sm:text-[9px] font-black text-slate-300 uppercase italic">ID: {member._id}</span>
                  </div>
               </div>
            ))}
         </div>
          ...
          {showAddModal && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
               <div className="bg-white w-full max-w-3xl rounded-[2rem] sm:rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[95vh] sm:max-h-[90vh]">
                  <div className="bg-blue-600 p-6 sm:p-8 text-white flex items-center justify-between shrink-0">
                     <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        <h3 className="text-lg sm:text-xl font-black uppercase tracking-widest leading-tight">Enroll Advanced Associate</h3>
                     </div>
                     <button onClick={() => setShowAddModal(false)} className="p-2 hover:bg-white/20 rounded-full transition-colors shrink-0"><X size={20} sm:size={24} /></button>
                  </div>
                  ...
                  <div className="overflow-y-auto p-8 md:p-12 custom-scrollbar">
                     <form onSubmit={handleAddSubmit} className="space-y-10">
                        ...
                        <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 text-white font-black py-4 sm:py-6 rounded-2xl sm:rounded-3xl shadow-2xl hover:bg-blue-700 transition-all uppercase tracking-[0.2em] sm:tracking-[0.3em] text-[10px] sm:text-[11px] flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
