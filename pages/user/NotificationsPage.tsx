...
export const UserNotifications: React.FC = () => {
  ...
  const notifications = useMemo(() => {
    return db.notifications.getByUser(user?.id || 'global');
  }, [user]);
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? (
          ...
          filtered.map(notif => (
            <div key={notif.id} className={`p-8 bg-white rounded-[2.5rem] border transition-all flex items-start gap-6 group cursor-pointer ${notif.read ? 'border-slate-100 opacity-60 hover:opacity-100' : 'border-indigo-100 shadow-xl shadow-indigo-100/30'}`}>
               ...
               <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                     ...
                     <span className="text-[9px] font-black text-slate-300 uppercase shrink-0">#{notif.id.split('-')[1]}</span>
                  </div>
               ...
               <button className="mt-1 p-2 text-slate-300 hover:text-rose-500 transition-colors opacity-0 group-hover:opacity-100">
                  <Trash2 size={18} />
               </button>
            </div>
          ))
