...
export const Wallet: React.FC = () => {
  ...
  const handleAddCredits = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ========== STRIPE TOP-UP (Real Integration) ==========
      if (formData.method === 'Stripe') {
        const response = await callBackendAPI('/api/stripe/wallet-topup', {
          userId: userId,
          amount: amount,
      ...
      // ========== PAYFAST TOP-UP (Real Integration) ==========
      if (formData.method === 'PayFast') {
        const response = await callBackendAPI('/api/payfast/wallet-topup', {
          userId: userId,
          amount: amount,
          currency: currency.code
        });
      ...
      if (formData.method === 'Manual') {
        ...
        setTimeout(() => {
          ...
          setFormData({ amount: '', method: 'Stripe' });
        }, 2000);
      }

    } catch (error: any) {
      console.error('Wallet top-up error:', error);
      alert(error.message || 'Failed to initiate wallet top-up. Please try again.');
      setIsProcessing(false);
    }
  };
