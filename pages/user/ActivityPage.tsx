...
export const UserActivity: React.FC = () => {
   ...
   useEffect(() => {
      const loadLogs = async () => {
         setIsLoading(true);
         try {
            const data = await callBackendAPI('/activities', null, 'GET');
            setActivityLogs(data || []);
         } catch (error) {
            console.error('Failed to load activity trace:', error);
         } finally {
            setIsLoading(false);
         }
      };
            ...
            <div className="relative z-10 space-y-0 before:absolute before:left-6 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
               ...
               {filteredLogs.length === 0 && !isLoading ? (
                  ...
                  filteredLogs.map((log, idx) => (
                     <div key={log._id} className="relative flex gap-10 pb-12 last:pb-0 animate-in slide-in-from-left-4" style={{ animationDelay: `${idx * 100}ms` }}>
