...
import { Rocket, Plus, Edit2, Trash2, CheckCircle2, ShieldCheck, Users, Zap, DollarSign, ArrowUpRight, TrendingUp, Loader2 } from 'lucide-react';
...
export const GlobalPlans: React.FC = () => {
  ...
  const getTenantCount = (planName: string) => {
    return users.filter(u => u.planName === planName).length;
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         {plans.map(plan => (
           <div key={plan._id} className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-8 flex flex-col justify-between group hover:border-indigo-500/50 transition-all border-b-8" style={{ borderBottomColor: plan.price > 5 ? '#f59e0b' : plan.price > 0 ? '#6366f1' : '#64748b' }}>
              <div className="space-y-6">
                 ...
                 <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Tenant Capacity: {getTenantCount(plan.name)} Nodes</p>
                 </div>
              </div>
              <div className="mt-10 pt-8 border-t border-white/5 space-y-4">
                 <div className="flex justify-between items-center text-[9px] font-black text-slate-400 uppercase">
                    <span>Repairs Node</span>
                    <span className="text-white">{plan.limits?.repairsPerMonth >= 999 ? '∞' : plan.limits?.repairsPerMonth || 0}</span>
                 </div>
                 <div className="flex justify-between items-center text-[9px] font-black text-slate-400 uppercase">
                    <span>Inventory Node</span>
                    <span className="text-white">{plan.limits?.inventoryItems >= 999 ? '∞' : plan.limits?.inventoryItems || 0}</span>
                 </div>
                 <div className="flex justify-between items-center text-[9px] font-black text-slate-400 uppercase">
                    <span>AI Logic</span>
