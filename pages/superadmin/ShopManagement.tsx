...
import { 
  ...
  TrendingUp, Activity, BarChart3, Globe, ShieldCheck, Loader2
} from 'lucide-react';
...
export const ShopManagement: React.FC = () => {
  ...
  const loadShops = async () => {
    try {
      setIsLoading(true);
      const data = await callBackendAPI('/api/superadmin/shops', null, 'GET');
      setShops(data || []);
    } catch (error) {
  ...
  const toggleShopStatus = async (shopId: string, currentStatus: string) => {
    ...
    try {
      await callBackendAPI(`/superadmin/users/${shopId}/status`, { status: nextStatus }, 'PUT');
      loadShops();
    } catch (error) {
    ...
    <div className="space-y-10 animate-in fade-in duration-700 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 flex flex-col gap-2">
           ...
           <h4 className="text-3xl font-black text-white">{shops.filter(s => s.planName && s.planName.toLowerCase() !== 'free trial').length}</h4>
           <div className="flex items-center gap-2 mt-2">
        ...
        <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 flex flex-col gap-2">
           ...
           <h4 className="text-3xl font-black text-white">{currency.symbol}{shops.reduce((a,b) => a+(b.totalRevenue||0), 0).toLocaleString()}</h4>
           <div className="flex items-center gap-2 mt-2">
            ...
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-white/5">
                  {filteredShops.map(shop => (
                    <tr key={shop._id} className="hover:bg-white/5 transition-all group">
                       ...
                       <td className="px-10 py-7 text-right">
                          <p className="text-base font-black text-white tracking-tighter">{currency.symbol}{(shop.totalRevenue || 0).toLocaleString()}</p>
