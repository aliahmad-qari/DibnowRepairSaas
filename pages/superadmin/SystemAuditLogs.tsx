...
import { ScrollText, Search, Filter, Terminal, ShieldCheck, Clock, Download, Database, Server, User, Globe, Hash, Info, Loader2, Lock } from 'lucide-react';
...
export const SystemAuditLogs: React.FC = () => {
  ...
  const loadLogs = async () => {
    try {
      setIsLoading(true);
      const data = await callBackendAPI('/api/superadmin/audit-logs', null, 'GET');
      setLogs(data || []);
    } catch (error) {
    ...
    <div className="space-y-10 animate-in fade-in duration-700 relative">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
         {[
           ...
           { label: 'Event Sync', val: '99.9% Latency', icon: Terminal, color: 'text-blue-500' },
              ...
              {filteredLogs.map((log) => (
                <tr key={log._id} className="hover:bg-white/5 transition-all group">
                  <td className="px-10 py-6">
                    <span className="font-mono text-[10px] font-black text-indigo-400">#{log._id?.slice(-8)}</span>
                  </td>
                  <td className="px-6 py-6">
                    <div className="flex items-center gap-3">
                       <div className="w-8 h-8 rounded-lg bg-white/10 text-slate-400 flex items-center justify-center font-black text-[9px] border border-white/5">{(log.userName || 'S').charAt(0)}</div>
                  ...
                  <td className="px-10 py-6">
                    <div className="space-y-1">
                       ...
                       <p className="text-[10px] text-slate-500 italic lowercase tracking-tight">"{log.details || log.description}"</p>
                    </div>
                  </td>
                  <td className="px-6 py-6">
                    <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest border-b border-indigo-500/30">{log.module || 'System'}</span>
                  </td>
