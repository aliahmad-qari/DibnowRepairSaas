...
export const GlobalAnnouncements: React.FC = () => {
  ...
  const loadAnnouncements = async () => {
    try {
      setIsLoading(true);
      const data = await callBackendAPI('/api/superadmin/announcements', null, 'GET');
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      await callBackendAPI('/api/superadmin/announcements', formData, 'POST');
      setShowModal(false);
  ...
  const deleteAnnouncement = async (id: string) => {
    try {
      await callBackendAPI(`/superadmin/announcements/${id}`, null, 'DELETE');
      loadAnnouncements();
    } catch (error) {
         ...
         <div className="lg:col-span-8 space-y-8">
            <div className="bg-white/5 border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl border-b-8 border-b-indigo-600">
               ...
               <div className="divide-y divide-white/5">
                  {announcements.map(a => (
                    <div key={a._id} className="p-8 flex items-start gap-6 hover:bg-white/5 transition-all group">
                       ...
                       <div className="flex-1 space-y-2">
                          <div className="flex items-center justify-between">
                             ...
                             <span className="text-[9px] font-bold text-slate-500 uppercase">{new Date(a.createdAt).toLocaleDateString()}</span>
                          </div>
                          <p className="text-sm font-medium text-slate-400 leading-relaxed uppercase tracking-tighter">"{a.message}"</p>
                          <div className="pt-4 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                             <span className="px-3 py-1 bg-white/5 rounded-lg text-[8px] font-black text-indigo-400 uppercase border border-white/5">{a.type}</span>
                             <button onClick={() => deleteAnnouncement(a._id)} className="text-slate-500 hover:text-rose-500 transition-colors"><Trash2 size={16}/></button>
                          </div>
                       </div>
                    </div>
                  ))}
