...
export const AdminManagement: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedAdmin) {
      db.adminUsers.update(selectedAdmin.id, formData);
      db.audit.log({
  ...
  const handleToggleStatus = (admin: any) => {
    ...
    db.adminUsers.update(admin.id, { is_disabled: nextStatus });
    setAdmins(db.adminUsers.getAll());
                    ...
                    <tr key={admin.id} className="hover:bg-white/5 transition-all group">
                       ...
                       <td className="px-6 py-7 text-center">
                          <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border inline-flex items-center gap-2 transition-all ${
                            !admin.is_disabled ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'
                          }`}>
                          ...
                          {PERMISSION_NODES.map(node => (
                            <button 
                              ...
                              className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 text-center ${
                                formData.permissions.includes(node.id as Permission) 
