...
import { LifeBuoy, Search, Filter, ShieldCheck, Clock, CheckCircle2, AlertCircle, X, ChevronRight, Hash, MessageSquare, Terminal, User, Building, Loader2 } from 'lucide-react';
...
export const GlobalSupport: React.FC = () => {
  ...
  const loadTickets = async () => {
    try {
      setIsLoading(true);
      const data = await callBackendAPI(`/superadmin/support/tickets?status=${filter}`, null, 'GET');
      setTickets(data || []);
    } catch (error) {
         ...
         <div className="overflow-x-auto">
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-white/5">
                  {tickets.map(t => (
                    <tr key={t._id} className="hover:bg-white/5 transition-all group cursor-pointer">
