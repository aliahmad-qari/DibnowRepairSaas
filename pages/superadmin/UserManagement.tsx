...
import { 
  ...
  Terminal, ShieldCheck, Mail, Database, X, ArrowUpRight, Loader2
} from 'lucide-react';
...
export const GlobalUserManagement: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      alert('Failed to load data. Please check: 1) Backend is running on port 5002, 2) You are logged in as superadmin');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const toggleStatus = async (userId: string, currentStatus: string) => {
    ...
    try {
      await callBackendAPI(`/api/superadmin/users/${userId}/status`, { status: nextStatus }, 'PUT');
      loadData();
    } catch (error) {
  ...
  const updatePlan = async (userId: string, planId: string) => {
    ...
    try {
      await callBackendAPI(`/api/superadmin/users/${userId}/plan`, { 
        planId, 
                    ...
                    <tr key={u._id} className="hover:bg-white/5 transition-all group">
                       ...
                       <td className="px-10 py-7">
                          <div className="flex items-center gap-3">
                             ...
                             >
                                {plans.map(p => <option key={p._id} value={p._id} className="bg-slate-900">{p.name}</option>)}
                             </select>
              ...
              <div className="p-10 border-b border-white/5 flex items-center justify-between bg-white/5">
                 <div className="flex items-center gap-6">
                    ...
                    <div>
                       ...
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Unique Identifier: {selectedUser._id}</p>
                    </div>
                 </div>
