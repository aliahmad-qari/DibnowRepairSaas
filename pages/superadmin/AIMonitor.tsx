...
export const AIMonitor: React.FC = () => {
   ...
   const invokeAIAudit = async () => {
      ...
      `;
         ...
         const data = await aiService.generateJson(prompt, "System: Platform Security AI Forensic Audit");
         if (data) {
                        ...
                        {forensicReport.anomalies?.map((risk: any, i: number) => (
                           <div key={i} className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] shadow-sm space-y-4 hover:border-indigo-500/50 transition-all group">
                              <div className="flex items-center justify-between">
                                 ...
                                 <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border ${risk.severity === 'High' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20 animate-pulse' : 'bg-white/5 text-slate-500 border-white/10'
