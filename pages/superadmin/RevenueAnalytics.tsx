...
import { 
  ...
  Scale, PieChart as PieIcon, Landmark, ShieldCheck, Loader2
} from 'lucide-react';
...
export const RevenueAnalytics: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      const [overviewData, chartDataRes] = await Promise.all([
        callBackendAPI('/api/superadmin/revenue/overview', null, 'GET'),
        callBackendAPI('/api/superadmin/revenue/chart', null, 'GET')
      ]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-8 space-y-4">
           ...
           <div>
              ...
              <h4 className="text-3xl font-black text-white tracking-tighter">{currency.symbol}{(overview?.totalRevenue || 0).toLocaleString()}</h4>
           </div>
        </div>
        <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-8 space-y-4">
           <div className="flex items-center justify-between">
           ...
           <div>
              ...
              <h4 className="text-3xl font-black text-white tracking-tighter">{currency.symbol}{(overview?.avgTransaction || 0).toFixed(2)}</h4>
           </div>
        </div>
        <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-8 space-y-4">
           <div className="flex items-center justify-between">
           ...
           <div>
              ...
              <h4 className="text-3xl font-black text-white tracking-tighter">{currency.symbol}{(overview?.totalRefunds || 0).toLocaleString()}</h4>
           </div>
        </div>
        <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-8 space-y-4">
           <div className="flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         <div className="bg-white/5 border border-white/10 rounded-[3rem] p-10">
            ...
            <div className="h-64 w-full">
               <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                     <Pie data={gatewayData} innerRadius={60} outerRadius={85} paddingAngle={8} dataKey="value" stroke="none">
