...
import { Coins, Globe, Plus, RefreshCw, Scale, ShieldCheck, MapPin, Search, Trash2, ArrowRightLeft, Loader2 } from 'lucide-react';
...
export const GlobalCurrencies: React.FC = () => {
  ...
  const loadCurrencies = async () => {
    try {
      setIsLoading(true);
      const data = await callBackendAPI('/api/superadmin/currencies', null, 'GET');
      setCurrencies(data || []);
    } catch (error) {
  ...
  const deleteCurrency = async (id: string) => {
    try {
      await callBackendAPI(`/superadmin/currencies/${id}`, null, 'DELETE');
      loadCurrencies();
    } catch (error) {
         ...
         <div className="lg:col-span-8 space-y-6">
            <div className="bg-white/5 border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl">
               <div className="p-8 border-b border-white/5 bg-white/5 flex items-center justify-between">
                  ...
                  <button onClick={loadCurrencies} className="p-3 bg-white/5 hover:bg-indigo-600 text-slate-400 hover:text-white rounded-xl transition-all"><RefreshCw size={18} /></button>
               </div>
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     ...
                     <tbody className="divide-y divide-white/5">
                        {currencies.map(c => (
                          <tr key={c._id} className="hover:bg-white/5 transition-all group">
                             ...
                             <td className="px-10 py-7 text-right">
                                <div className="flex items-center justify-end gap-3">
                                   ...
                                   <button onClick={() => deleteCurrency(c._id)} className="p-2 text-slate-500 hover:text-rose-500 transition-colors"><Trash2 size={16}/></button>
                                </div>
                             </td>
                          </tr>
                        ))}
